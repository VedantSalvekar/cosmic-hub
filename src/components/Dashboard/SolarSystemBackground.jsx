...
/**
 * Global orbit controls so mouse events still work when UI overlays sit on top
 */
const SolarSystemControls = () => {
  const { gl } = useThree();
  const domElement =
    typeof document !== "undefined" ? document.body : gl.domElement;
...
const SolarSystemBackground = ({ isBlurred = false }) => {
  return (
    <div
      className={`fixed inset-0 w-full h-full z-0 bg-black transition-all duration-500 ${
        isBlurred ? "blur-sm" : ""
      }`}
    >
      <Canvas
